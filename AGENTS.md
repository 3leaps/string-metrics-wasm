...
Co-Authored-By: Claude Code <noreply@3leaps.dev>
```
...
4. Co-authored-by trailer — use the **3 Leaps-controlled** address `noreply@3leaps.dev`, **not** a
   model-vendor address. Vendor noreply emails can be registered by unrelated GitHub accounts, which
   then appear as repo "contributors"; a 3 Leaps address avoids that mis-attribution.
5. Keep internal planning IDs (e.g. task numbers) out of commit messages — they are opaque to public
   readers (a PR number like `(#5)` is fine).
